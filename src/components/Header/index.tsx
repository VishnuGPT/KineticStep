          ...
          <div className="relative -mx-4 flex items-center justify-between">
                ...
                {mounted ? (
                  <>
                    <Image
                      ...
                      alt="logo"
                      width={40} // Adjusted size to sit better next to text
                      height={40}
                      className="max-h-[40px] w-auto object-contain"
                      priority
                    />
                    <span className="text-xl font-bold text-black dark:text-white lg:text-2xl">
                      KineticStep
                    </span>
                  </>
                ) : (
            ...
            <div className="flex w-full items-center justify-between px-4">
                          ...
                          <>
                            ...
                            <div className={`submenu dark:bg-dark relative top-full left-0 rounded-sm bg-white transition-[top] duration-300 group-hover:opacity-100 lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-[250px] lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${openIndex === index ? "block" : "hidden"}`}>
                              {menuItem.submenu?.map((submenuItem, i) => (
                                <Link href={submenuItem.path} key={i} className="text-dark hover:text-primary block rounded-sm py-2.5 text-sm lg:px-3 dark:text-white/70 dark:hover:text-white">
                                  {submenuItem.title}
                                </Link>
                              ))}
                            </div>
              ...
              <div className="flex items-center justify-end pr-16 lg:pr-0">
                <Link href="/signin" className="text-dark hidden px-7 py-3 text-base font-medium hover:opacity-70 md:block dark:text-white">Sign In</Link>
                <Link href="/signup" className="ease-in-up shadow-btn hover:shadow-btn-hover bg-primary hover:bg-primary/90 hidden rounded-xs px-8 py-3 text-base font-medium text-white transition duration-300 md:block md:px-9 lg:px-6 xl:px-9">Sign Up</Link>
                <div>
                  <ThemeToggler />
                </div>
              </div>
